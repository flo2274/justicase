...
class CaseNotifier extends StateNotifier<CaseState> {
  CaseNotifier()
      : super(CaseState(allCases: [], userCases: [], isLoading: true));
  ...
  Future<void> fetchAllCases() async {
    try {
      ...
      state = state.copyWith(
          isLoading: false, errorMessage: 'Fehler beim Abrufen der Fälle: $e');
    }
  }
  ...
  Future<bool?> createCase(Case newCase) async {
    try {
      ...
      state =
          state.copyWith(errorMessage: 'Fehler beim Erstellen des Falls: $e');
    }
    return null;
  }
}
